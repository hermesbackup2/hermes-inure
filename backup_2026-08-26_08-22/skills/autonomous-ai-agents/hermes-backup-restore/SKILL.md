...
- GitHub **Push Protection / secret scanning** blocks commits containing tokens. `state.db` (Hermes session/conversation DB) embeds Telegram bot tokens and old PATs from chat history, and `.env` embeds live secrets — pushing either triggers `GH013: Repository rule violations / Push cannot contain secrets` and the push is rejected. ALWAYS exclude `state.db` AND `.env` from the snapshot (the original skill text only warned about `.env`). If an earlier bad commit already landed in history, `rm -rf .git && git init` to start a clean history, then push. Verify remotely via the GitHub API, not just local git success.
